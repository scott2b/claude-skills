...
1. **Dates and timestamps**: Claude does not have a built-in real-time clock tool and may hallucinate or guess dates incorrectly. When a skill needs the current date:
   ...
   - Example instruction: "Get the current date using `date +%Y-%m-%d` command"

2. **Progressive disclosure**: Keep SKILL.md lean (<5k words) by moving detailed theory, examples, and reference material to `references/` files. Only include essential procedural instructions in SKILL.md.
